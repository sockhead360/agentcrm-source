...
// ── AI LOCK ────────────────────────────────────────────────────────────────────
// The AI is not ready to share, so it is gated by a password that travels WITH THE BUILD
// rather than living in each user's database. Consequences that matter:
//   * a fresh install is locked from the first launch, not wide open;
//   * clearing or editing the local database cannot unlock it;
//   * an update restores the lock if anyone tampers with it;
//   * the plaintext exists nowhere - not in the app, not in the repo, not in any DB.
// scrypt with a random salt, so the committed hash cannot be reversed or usefully
// brute-forced even by someone reading the source repo.
// GENERATED by `node scripts/set-ai-lock.js` - do not hand-edit the line below.
const AI_LOCK = { salt: 'ca89fe025f19445b81345aa23e995557b9a027bdc7b1690a984be9721316296b', hash: '0b4e087cf36b23ad734a9b622e786a8acad80b58a6eb8e9188e4f1759d2857960c15b2cd675bbf33c263c3cafaf5c0d09693632fdfbc48924345b3bdf2fb9c3d', n: 16384, r: 8, p: 1, keylen: 64 };

// A successful unlock is remembered in MAIN-PROCESS MEMORY ONLY, never persisted and never
// exposed to the renderer, so devtools cannot fake it and it dies with the process.
let aiUnlockedUntil = 0;
const AI_UNLOCK_WINDOW_MS = 30 * 60 * 1000;
const aiLockConfigured = () => !!(AI_LOCK.salt && AI_LOCK.hash);
const aiIsUnlocked = () => !aiLockConfigured() ? false : Date.now() < aiUnlockedUntil;

function verifyAiLock(password) {
  if (!aiLockConfigured() || !password) return false;
  try {
    const derived = crypto.scryptSync(String(password), Buffer.from(AI_LOCK.salt, 'hex'),
      AI_LOCK.keylen, { N: AI_LOCK.n, r: AI_LOCK.r, p: AI_LOCK.p });
    const expected = Buffer.from(AI_LOCK.hash, 'hex');
    return derived.length === expected.length && crypto.timingSafeEqual(derived, expected);
  } catch (_) { return false; }
}

// Settings that turn the AI on or change how far it can act. Locked together: unlocking to
// pick a level must not also silently permit enabling auto-submit.
const AI_LOCKED_SETTINGS = ['aiEnabled', 'aiLevel', 'aiAutoSubmit'];
...
// A short sale that HASN'T HAPPENED YET is a live pre-distress window, not a cold deal type
// (Chris, 2026-08-03). "I have a client that needs to get out of her home 940 W Central. It
// would most likely be result in a short sale" is a motivated seller WITH an address whose
// property is not a short sale today — exactly the pre-foreclosure / future-auction spirit
// already carved out above. Only the CURRENT state cold-closes; the whole business is buying
// in the window before the distressed outcome lands.
//
// Tense/conditional markers must appear within a short span before "short sale" so a plain
// "I have a short sale" is untouched. Mirrors PRE_LISTING_RE beating ON_MARKET_LISTED_RE.
const FUTURE_SHORT_SALE_RE = /\b(?:would|will|'ll|could|might|may|maybe|probably|likely|possibly|going to|gonna|end(?:s|ed|ing)?\s+up|result(?:s|ed|ing)?\s+in|turn(?:s|ed|ing)?\s+into|become|becomes|becoming|head(?:ed|ing)?\s+(?:for|to|towards?)|if|unless|before)\b[^.!?]{0,70}?\bshort\s+sale/gi;

function isExcludedDealType(body = '') {
  const raw = body || '';
  // Strip future/conditional short-sale mentions, then judge what's left. Anything else in
  // the message (commercial, REO, FSBO, a CURRENT short sale) still cold-closes normally.
  const stripped = raw.replace(FUTURE_SHORT_SALE_RE, ' ');
  return EXCLUDED_DEAL_TYPE_RE.test(stripped);
}
...
async function extractLeadDetails(messages, apiKey) {
  ...
  const system =
    ...
    "CRITICAL: every field ONLY counts what the AGENT stated (lines prefixed 'Agent:'). NEVER extract a value that only appears in one of YOUR OWN messages (lines prefixed 'You:') — your own outbound texts often paraphrase or nickname the property ('your property on Main St', 'that place off 5th') and none of that is the agent giving the real address. If the address only appears in a 'You:' line, return null for it instead of that paraphrase.\n" +
    "address: the FULL street address exactly as the agent wrote it, including unit/apt number, city, state, and zip if the agent gave them (e.g. '607 E Hill', '276 Pinon Rd', '735 Main St #3, Osceola, MO 64776'). Prefer the most complete version the agent gave anywhere in the conversation, not a shortened later reference. null if not found.\n" +
...
async function autoSubmitLead(conv, contact, settings) {
  try {
    ...
    // Full history, not a trailing slice: a deal can go quiet for weeks between follow-up
    // touches, so the agent's original address (and price) is often many messages back by
    // the time the thread actually graduates to hot. A narrow window silently drops it and
    // the extractor falls back to whatever's in the recent window, including a paraphrase
    // from one of OUR OWN follow-up texts.
    const details = await extractLeadDetails(allMsgs, settings.claudeApiKey);
    ...
    // Also stamp the conversation itself so the buddy-list preview can show the address
    // at a glance without opening the thread.
    db.setConversationAddress(conv.id, details.address);
...
async function generateAiReply(msgBody, history, contact, settings) {
  ...
  const systemStatic =
    ...
    `AGENT LINGO GLOSSARY (comprehension only — so you never misread an agent's jargon or abbreviations. This does NOT add new reply behavior: where a specific scripted reply already exists above for one of these terms (EMD, POF, BBA, COE, inspection/option period, commission, etc.), that scripted reply still governs):\n` +
    `- ARV = after repair value, the resale value once repaired. MAO = maximum allowable offer (ARV x market% - repairs - assignment fee).\n` +
    `- EMD = earnest money deposit. PA/PSA = purchase agreement / purchase and sale agreement, the buyer-seller contract. BBA = buyer broker agreement, the agent's buyer-representation agreement.\n` +
    `- Assignment = transferring contract rights to a new buyer for a fee. Assignor = the one transferring (often the wholesaler). Assignee = the one receiving (often the end buyer). Assignment fee = the wholesaler's fee for the transfer, not automatically net profit.\n` +
    `- CTC/C2C = clear to close (title/escrow conditions met, ready to close). COE = close of escrow, the closing date. DD = due diligence, the buyer's investigation period. IP = inspection period, same idea as our own "option period" answer. TC = transaction coordinator, tracks paperwork and dates. POF = proof of funds.\n` +
    `- Addendum = added contract terms. Amendment = a change to a term already in the signed contract. Contingency = a condition that lets a party proceed, renegotiate, or cancel. Title search = ownership/lien records review. Escrow = neutral third party holding funds/docs until closing. Double close = the wholesaler buys then resells in a second closing, often same day.\n` +
    `- M2M = month-to-month rental. Comps = comparable recent sales used to value a property. MLS = the on-market listing database (an MLS mention is an on-market signal per the on-market rules elsewhere). DOM = days on market. SFH/SFR = single-family home. MFH = multi-family (duplex/triplex/apartment). HOA = homeowners association. As-is = sold in current condition, no repairs promised.\n` +
    `- JV = joint venture, two wholesalers/businesses splitting a deal. Acquisitions = seller-side deal work (finding and negotiating leads). Dispo = marketing a contracted deal to buyers. Cash buyer = a buyer without a financing contingency. Rent roll = a tenant/rent/lease summary for an income property.\n\n` +
    `CATEGORIES:\n` +
...
// ══════════════════════════════════════════════════════════════════════════════
// HOT FOLLOW-UP AGENT — the second agent. Owns a lead only AFTER an offer goes out.
//
// Everything below is deliberately dumb: it presses for an answer on a schedule and
// keeps a pulse. It never negotiates, never composes language, and never picks a phrase
// a human didn't write. It is entirely separate from the warm agent above — it does not
// read human_replied and does not call generateAiReply.
// ══════════════════════════════════════════════════════════════════════════════

// Chris's hand-written libraries. Used VERBATIM — never rewritten, never model-generated.
// {streetNumber + streetName} in the source doc means the two joined by a space.
const HFU_LIB_PRESS = [
  'Were you able to present my offer?',
  'Hey {firstName} were you able to get my offer to the seller?',
  'Any feedback or counter offer from the seller?',
  'Have you heard back from the seller?',
  '{firstName} did you get my last message?',
  'My team is still interested in the property on {streetName}.. Any word back from the seller?',
  'Hey {firstName} is the property on {streetName} still available?',
  '{firstName} Any updates regarding our offer on {streetNumber} {streetName}?',
  'Hey {firstName}, circling back on {streetName}..any word from the seller?',
  '{firstName} any luck connecting with the seller on {streetNumber} {streetName}?',
  'Following up on my offer.. did the seller hear our offer with our terms?',
  'Hey {firstName}, still waiting to hear back on {streetName}. Anything yet?',
  '{firstName} were you able to run my offer by the seller?',
  'Any movement on {streetName}? We’re ready to move when they are.',
  'Hey {firstName}, was the seller a yes, no, or maybe on our offer?',
  'Checking in on {streetNumber} {streetName}..is the seller still open to an offer?',
  '{firstName} just making sure my offer made it over to the seller?',
  'Hey {firstName}, any counter on {streetName}? We’ve got some flexibility.',
  '{firstName} where do we stand on {streetName}?',
  'Still have funds set aside for {streetName}..any update from the seller?',
];

const HFU_LIB_GENERIC = [
  '{firstName} any updates on your end?',
  'Hey {firstName}, where are we at with this one?',
  '{firstName} anything new since we last talked?',
  'Just checking in on {streetNumber} {streetName} any movement?',
  'Any word yet, {firstName}?',
  'Hey {firstName}, what’s the latest on {streetNumber} {streetName}?',
  '{firstName} any news for me regarding {streetNumber} {streetName}?',
  'Hey {firstName}, how’s this one looking on your end?',
  '{firstName} where do we stand on {streetNumber} {streetName}?',
  'Any progress, {firstName}?',
  'Hey {firstName}, did anything come up on this one?',
  '{firstName} checking back in — anything change?',
  'Any movement on {streetNumber} {streetName}?',
  'Hey {firstName}, hear anything new?',
  '{firstName} still nothing yet?',
  'Following up on {streetNumber} {streetName}..where are we at?',
  'Hey {firstName}, anything to report?',
  '{firstName} quick check in — any updates?',
  'Any updates on {streetNumber} {streetName} today, {firstName}?',
  'Hey {firstName}, circling back. Anything new?',
];

const HFU_LIB_RED = [
  'Hey {firstName} just wanted to check back in regarding {streetNumber} {streetName} has anything changed? And wanted to see if you’ve come across any other off-market fixer uppers since we last talked.',
  'Did {streetNumber} {streetName} end up selling? If so, let me know if any other off-market fixer uppers have come across your desk this week!',
  'Hope you’re doing well {firstName}. Has anything changed with {streetNumber} {streetName}? My team was asking me about it yesterday, I figured I’d check in and see what was going on and to see if you’ve come across any other off-market properties that need work.',
  'Hey {firstName} I’m curious, what ended up happening with {streetNumber} {streetName}? Any other fixer uppers that you know of coming to the market?',
  'Hey! Has {streetNumber} {streetName} sold yet? We’re looking to pick up another off-market fix and flip property this week',
  'Wanted to check back in one more time about {streetNumber} {streetName} any update? Also do you have anything else that’s off-market and needs work?',
];

const HFU_LIBS = { press: HFU_LIB_PRESS, generic: HFU_LIB_GENERIC, red: HFU_LIB_RED };

// Cadence in hours. Press is color-blind and daily; Tier 2 is set by the dot.
const HFU_INTERVAL_HOURS = { press: 24, '🟢': 24, '🟡': 48, '🔴': 336 };
const HFU_RED_MAX_DAYS = 90;

// Split a free-text address into the tokens the libraries need. Real values run from
// "735 Main St #3, Osceola, MO 64776" to "103-105 Brandon Ave" to
// "3957 Ashland ave in cantonment right off of 9 mile", so this stays conservative:
// a leading number (possibly hyphenated) plus the words up to the first comma, with a
// street suffix if one is present. Anything it can't read returns nulls and the caller
// falls back to a token-free line rather than sending a blank.
const HFU_STREET_SUFFIX = /\b(st|street|ave|avenue|rd|road|dr|drive|ln|lane|blvd|boulevard|way|ct|court|pl|place|cir|circle|trl|trail|pkwy|parkway|hwy|highway|ter|terrace)\b\.?/i;
function parseAddressTokens(address) {
  const raw = String(address || '').trim();
  if (!raw) return { streetNumber: null, streetName: null };
  const head = raw.split(',')[0].trim();
  const m = head.match(/^(\d+(?:\s*-\s*\d+)?)\s+(.+)$/);
  if (!m) return { streetNumber: null, streetName: null };
  const streetNumber = m[1].replace(/\s+/g, '');
  let rest = m[2].trim();
  // Trim trailing unit designators and anything past the street suffix, so a chatty
  // address doesn't drag "in cantonment right off of 9 mile" into the message.
  const sfx = rest.match(HFU_STREET_SUFFIX);
  if (sfx) rest = rest.slice(0, sfx.index + sfx[0].length).trim();
  rest = rest.replace(/\s*#.*$/, '').replace(/[.,]+$/, '').trim();
  if (!rest) return { streetNumber: null, streetName: null };
  return { streetNumber, streetName: rest };
}

function hfuTokens(conv, contact) {
  const { streetNumber, streetName } = parseAddressTokens(conv.address);
  const first = (contact?.first_name || contact?.name?.split(' ')[0] || '').trim();
  return { firstName: first || null, streetNumber, streetName };
}

function hfuFill(line, tokens) {
  return line.replace(/\{(\w+)\}/g, (_, k) => tokens[k] ?? '');
}

// True when every token the line needs is actually available — the §9 guard against
// sending a message with a blank where a name should be.
function hfuLineIsSendable(line, tokens) {
  const needed = [...line.matchAll(/\{(\w+)\}/g)].map(m => m[1]);
  return needed.every(k => tokens[k] != null && String(tokens[k]).trim() !== '');
}

// Walk the library in order, skipping lines already used on this conversation and lines
// whose tokens are missing. Wraps to the start once exhausted (Chris's call — he'll add
// more phrases later), and reports the wrap so it's visible rather than silent.
// NOTE: there is deliberately no offer detection anywhere in this agent. Activation is
// the Offer Sent button and nothing else — Chris presses it himself, so inferring intent
// from message text would add a guess where a fact already exists.

// Pure core: given the line numbers already used, pick the next one. No DB, no state —
// so the simulator and the live scheduler provably run the SAME selection logic.
function pickHfuLinePure(usedLines, libraryKey, tokens) {
  const lib = HFU_LIBS[libraryKey];
  const used = new Set(usedLines);
  const sendable = lib.map((body, i) => ({ body, lineNo: i + 1 }))
                      .filter(x => hfuLineIsSendable(x.body, tokens));
  if (sendable.length === 0) return null; // e.g. a red lead with no address at all
  const fresh = sendable.filter(x => !used.has(x.lineNo));
  if (fresh.length > 0) return { ...fresh[0], wrapped: false };
  // Exhausted: restart from the top, least-used first.
  const counts = usedLines.reduce((acc, n) => (acc[n] = (acc[n] || 0) + 1, acc), {});
  const leastUsed = [...sendable].sort(
    (a, b) => (counts[a.lineNo] || 0) - (counts[b.lineNo] || 0) || a.lineNo - b.lineNo
  )[0];
  return { ...leastUsed, wrapped: true };
}

function pickHfuLine(convId, libraryKey, tokens) {
  return pickHfuLinePure(db.getUsedPhraseLines(convId, libraryKey), libraryKey, tokens);
}

// ── Hot follow-up SIMULATOR ───────────────────────────────────────────────────
// Deterministic dry-run of the whole cadence, compressed. No DB, no network, no Twilio —
// pure arithmetic over the same libraries and intervals the live scheduler uses, so what
// you see here is exactly what would go out. Powers both the in-app sandbox and the
// public Vercel demo.
//
// events: { answerOnDay, pauseOnDay, resumeOnDay, colorOnDay } — all optional, all in
// days since the offer. colorOnDay simulates the underwriters tagging it late.
function simulateHotFollowUp(opts = {}) {
  const {
    firstName = 'Piper', address = '910 E 8th Street, Pueblo CO 81001',
    color = null, days = 30,
    answerOnDay = null, pauseOnDay = null, resumeOnDay = null, colorOnDay = null,
  } = opts;

  const tokens = hfuTokens({ address }, { first_name: firstName });
  const timeline = [];
  const usedByLib = { press: [], generic: [], red: [] };
  let phase = 'press';
  let activeColor = colorOnDay === 0 ? color : (colorOnDay == null ? color : null);
  let nextDue = 0;
  let pausedFrom = null;
  let heldForColor = false;
  let touches = 0;

  const push = (day, type, extra = {}) => timeline.push({ day, type, phase, ...extra });
  push(0, 'armed', { note: 'Offer Sent pressed — Tier 1 begins, color-blind' });

  for (let day = 0; day <= days; day++) {
    if (colorOnDay != null && day === colorOnDay && color) {
      activeColor = color;
      push(day, 'colored', { note: `Tagged ${color} by underwriting` });
      if (phase === 'maintain' && heldForColor) { heldForColor = false; nextDue = day; }
    }
    if (answerOnDay != null && day === answerOnDay && phase === 'press') {
      push(day, 'answer', { note: 'Agent gives a REAL answer — Tier 1 closes' });
      pausedFrom = 'maintain';
      phase = 'paused';
      push(day, 'pause', { note: 'Auto-paused + flagged as news. Never auto-resumes.' });
      continue;
    }
    if (pauseOnDay != null && day === pauseOnDay && (phase === 'press' || phase === 'maintain')) {
      pausedFrom = phase; phase = 'paused';
      push(day, 'pause', { note: 'You pressed pause' });
      continue;
    }
    if (resumeOnDay != null && day === resumeOnDay && phase === 'paused') {
      phase = pausedFrom === 'press' ? 'press' : 'maintain';
      nextDue = day;   // clock re-anchors to now — no backfill
      push(day, 'resume', { note: `Resumed as ${phase} — clock re-anchored, missed slots not owed` });
    }
    if (phase === 'paused') continue;

    // Red retires itself at 90 days.
    if (phase === 'maintain' && activeColor === '🔴' && day >= HFU_RED_MAX_DAYS) {
      push(day, 'cold', { note: `${HFU_RED_MAX_DAYS} days reached — auto-cold, follow-up stops permanently` });
      break;
    }
    // Tier 2 with no color holds rather than guessing a cadence.
    if (phase === 'maintain' && !activeColor) {
      if (!heldForColor) { heldForColor = true; push(day, 'hold', { note: 'No color yet — holding. Nothing sends until it is tagged.' }); }
      continue;
    }
    if (day < nextDue) continue;

    const lib = phase === 'press' ? 'press' : (activeColor === '🔴' ? 'red' : 'generic');
    const pick = pickHfuLinePure(usedByLib[lib], lib, tokens);
    if (!pick) {
      push(day, 'skip', { note: `No sendable ${lib} phrase — missing ${Object.entries(tokens).filter(([, v]) => !v).map(([k]) => k).join(', ')}. Flagged for you.` });
      nextDue = day + 1;
      continue;
    }
    usedByLib[lib].push(pick.lineNo);
    touches++;
    push(day, 'touch', {
      library: lib, lineNo: pick.lineNo, wrapped: pick.wrapped,
      body: hfuFill(pick.body, tokens),
    });
    const iv = phase === 'press' ? HFU_INTERVAL_HOURS.press : HFU_INTERVAL_HOURS[activeColor];
    nextDue = day + Math.round(iv / 24);
  }
  return { timeline, touches, endedPhase: phase };
}

// ── Hot follow-up CLASSIFIER ──────────────────────────────────────────────────
// The one judgement this agent makes. After an offer goes out, every agent reply is
// exactly one of three things, and the whole state machine hangs off which:
//
//   answer — the seller's position came back. Tier 1 has done its job: STOP pressing,
//            flag it as news, hand the thread to a human. "Too low", "they accepted",
//            "they countered at 30", "seller said no" are all answers. An answer is not
//            the same as a good answer — a rejection is still an answer.
//   stall  — a reply that carries no seller position. Promises, apologies, logistics,
//            "I'll present it tonight", "haven't heard back yet", "seller is traveling".
//            The ball is still in the agent's court, so Tier 1 KEEPS PRESSING. This is
//            the case a naive "they replied, so stop" rule gets wrong, and it is the
//            single most common reply we get.
//   dead   — the property is no longer a live off-market deal we could win: sold, closed,
//            gone, seller withdrew it, or it went on the MLS. Follow-up stops for good.
//
// Deliberately NOT dead: under contract / pending / contingent. Those deals fall apart
// constantly and we want the backup position, so they classify as an answer and a human
// decides. The test to apply is always: is this still a live off-market deal we could win?
async function classifyHotFollowUpReply(body, apiKey) {
  const client = aiClient(apiKey);
  const response = await client.messages.create({
    model: 'claude-sonnet-5', thinking: { type: 'disabled' },
    max_tokens: 16,
    system:
      ...
      "If the reply mixes a stall with a real seller position (\"still waiting on him but he did say 20 is too low\"), that is an answer.\n\n" +
      "Reply with ONLY one word: answer, stall, or dead.",
    messages: [{ role: 'user', content: String(body || '') }],
  });
  const raw = aiText(response).toLowerCase();
  // Default to stall on anything unparseable: pressing one more time is recoverable,
  // wrongly declaring an answer silently hands a live lead to a human queue and stops
  // the chase, and wrongly declaring dead throws the deal away outright.
  const verdict = ['answer', 'dead', 'stall'].find(v => raw.includes(v)) || 'stall';
  return { verdict, raw };
}
...
// Which library a lead draws from, given its phase and dot.
function hfuLibraryFor(conv) {
  if (conv.hot_fu_state === 'press') return 'press';
  return conv.emoji === '🔴' ? 'red' : 'generic';
}

// Only these three are underwriter colors. Any other emoji on the thread (🤝 etc.) is a
// tag, not a deal grade, and does NOT set a cadence.
function hfuDealColor(conv) {
  return HFU_INTERVAL_HOURS[conv.emoji] && conv.emoji !== 'press' ? conv.emoji : null;
}

// Gap until the next touch. Press is color-blind daily (§5). Tier 2 is set entirely by
// the dot — there is no default. A maintain lead with no color HOLDS (see the scheduler);
// this function is only ever called once a cadence is known to exist.
function hfuIntervalHours(conv) {
  if (conv.hot_fu_state === 'press') return HFU_INTERVAL_HOURS.press;
  return HFU_INTERVAL_HOURS[hfuDealColor(conv)];
}

function hfuScheduleNext(convId, conv, fromSec = Math.floor(Date.now() / 1000)) {
  const next = fromSec + hfuIntervalHours(conv) * 3600;
  db.setHotFuState(convId, conv.hot_fu_state, { hot_fu_next_at: next });
  return next;
}

async function sendDueHotFollowUps(settings) {
  // Master AI switch governs this agent too (Chris's call): AI off behaves exactly like
  // the laptop being off — nothing fires, and on clock-in each lead resumes with ONE due
  // touch rather than a catch-up burst, because the schedule is re-anchored on send.
  if (settings.aiEnabled !== 'true' || !settings.claudeApiKey) return;
  if (parseInt(settings.aiLevel || '3', 10) < 3) return;
  if (!withinSendingHours(settings)) {
    log(`Hot follow-up: outside sending hours (${easternHourNow()}:00 ET) — deferring`);
    return;
  }

  const now = Math.floor(Date.now() / 1000);
  const due = db.getDueHotFollowUps(now);

  for (const conv of due) {
    try {
      // ── HARD RULE: no AI of any kind touches RED HOT. ──
      // Those are contracts and live deals. This is a defensive backstop, not the primary
      // guard (updateCategory stops follow-up the moment a lead is promoted) — it exists so
      // that no future change, migration, or hand-edited row can put an armed lead in front
      // of this loop while it sits in caliente.
      if (conv.category === 'caliente') {
        hfuFinish(conv, 'promoted_red_hot');
        log(`Hot follow-up: conv ${conv.id} is RED HOT — stopping, AI never touches those`);
        continue;
      }
      // Equally, anything that has left hot_lead has no business being pressed.
      if (conv.category !== 'hot_lead') {
        hfuFinish(conv, 'demoted_from_hot');
        log(`Hot follow-up: conv ${conv.id} is no longer hot (${conv.category}) — stopping`);
        continue;
      }

      // Red retires itself at 90 days — but only in Tier 2. Tier 1 is uncapped by design
      // (spec, "No cap"): a lead still being pressed for an answer never auto-cools, no
      // matter what dot underwriting hung on it. Without the phase gate a red tagged early
      // would go cold mid-press, which is the opposite of "it runs until it gets an answer
      // or until I stop it."
      if (conv.hot_fu_state === 'maintain' && conv.emoji === '🔴' && conv.hot_fu_armed_at &&
          (now - conv.hot_fu_armed_at) > HFU_RED_MAX_DAYS * 86400) {
        hfuFinish(conv, 'red_90day', { markCold: true });
        continue;
      }

      // ── Tier 2 with no color: HOLD. Do not guess a cadence. ──
      // Tier 1 is deliberately color-blind, so a lead can legitimately reach here before
      // the underwriters have graded it. Once we're past the answer, the dot is the only
      // thing that sets a rhythm — so the lead parks and waits for Chris rather than
      // inventing one. Re-checks hourly and starts the moment a dot appears.
      if (conv.hot_fu_state === 'maintain' && !hfuDealColor(conv)) {
        const last = db.getHotFollowUpLog(conv.id).slice(-1)[0];
        if (!last || last.status !== 'waiting_for_color') {
          db.logHotFollowUp({
            convId: conv.id, phase: 'maintain', library: '-', lineNo: 0,
            body: '[holding — no color tag yet, waiting on underwriting]',
            status: 'waiting_for_color',
          });
          db.setHasNews(conv.id, true);
          log(`Hot follow-up: conv ${conv.id} is in maintain with no color — holding until tagged`);
        }
        db.setHotFuState(conv.id, 'maintain', { hot_fu_next_at: now + 3600 });
        continue;
      }

      const contact = db.getContactById(conv.contact_id);
      const tokens = hfuTokens(conv, contact || conv);
      const library = hfuLibraryFor(conv);
      const pick = pickHfuLine(conv.id, library, tokens);

      // Nothing legal to send — every line in this library needs a token we don't have.
      // Red is the real case: all 6 of its lines embed the address. Surface it rather
      // than sending something malformed, and hold the cadence so it retries later.
      if (!pick) {
        db.logHotFollowUp({
          convId: conv.id, phase: conv.hot_fu_state, library, lineNo: 0,
          body: `[skipped — no sendable phrase: missing ${
            Object.entries(tokens).filter(([, v]) => !v).map(([k]) => k).join(', ') || 'tokens'
          }]`,
          status: 'skipped_no_phrase',
        });
        db.setHasNews(conv.id, true);
        log(`Hot follow-up: conv ${conv.id} has no sendable ${library} phrase — flagged`);
      ...
      if (pick.wrapped) {
        log(`Hot follow-up: conv ${conv.id} wrapped the ${library} library — repeating from the start`);
      }
      hfuScheduleNext(conv.id, conv, now);
    } catch (err) {
...
// frozen into the row here, which is why clearing the dot afterwards costs no history.
function hfuFinish(conv, outcome, { markCold = false } = {}) {
  try {
    const contact = db.getContactById(conv.contact_id) || {};
    const now = Math.floor(Date.now() / 1000);
    const armed = conv.hot_fu_armed_at || null;
    const parsed = parseAddressTokens(conv.address);
    db.recordHotLeadOutcome({
      conv_id: conv.id,
      contact_id: conv.contact_id,
      lead_submission_id: db.getLatestLeadSubmissionIdForContact(conv.contact_id),
      agent_name: contact.name || null,
      agent_phone: contact.phone || null,
      brokerage: contact.brokerage || null,
      agent_city: contact.city || null,
      agent_state: contact.state || null,
      property_address: conv.address || null,
      property_city: null,   // parsed market split is a later decision — see the plan
      property_state: null,
      asking_price: null,
      offer_amount: null,   // no detection — see the note where activation is defined
      color_at_end: conv.emoji || null,
      followup_outcome: outcome,
      armed_at: armed,
      ended_at: now,
      days_active: armed ? Number(((now - armed) / 86400).toFixed(2)) : null,
      touches_sent: db.countHotFollowUps(conv.id),
      last_phase: conv.hot_fu_state || null,
    });
    db.setHotFuState(conv.id, 'stopped', { hot_fu_next_at: null });
    if (markCold) {
      db.updateConversationCategory(conv.id, 'not_interested');
      db.cancelWarmDrips(conv.id);
      db.cancelPendingFollowUps(conv.id);
    }
    log(`Hot follow-up: conv ${conv.id} ended — ${outcome} (${db.countHotFollowUps(conv.id)} touches)`);
  } catch (err) {
    log(`Hot follow-up finish error on conv ${conv.id}: ${err.message}`);
  }
}
...
async function pollTwilio() {
  ...
  try {
    ...
    // Hot follow-up rides the same tick, and therefore inherits the ordering guarantee
    // above: any reply that arrived while the app was off is processed BEFORE this runs,
    // so a lead whose agent said "it sold" on Saturday is never texted on Monday.
    await sendDueHotFollowUps(settings);
...
ipcMain.handle('conversations:updateCategory', (_, { convId, category }) => {
  // Read BEFORE the write so we can tell what the lead was when the move happened.
  const prevConv = db.getConversationById(convId);
  ...
  // ── §8: moving a lead out of hot stops the hot follow-up agent, permanently. ──
  // Marking cold is the explicit case in the spec ("all auto follow-ups stop permanently"),
  // but ANY move off hot_lead means the same thing: the premise of this agent is a live
  // offer on a hot lead. Demoting to warm hands the thread back to the warm drip, and two
  // agents chasing one conversation is worse than either alone.
  //
  // RED HOT (caliente) is the most important case: those are contracts and live deals, and
  // NO AI touches them, ever. Promoting a lead there stops follow-up immediately — it is
  // not a failure, it is the deal graduating past anything automated.
  if (category !== 'hot_lead' && prevConv && prevConv.hot_fu_state && prevConv.hot_fu_state !== 'stopped') {
    const outcome =
      category === 'caliente' ? 'promoted_red_hot' :
      (category === 'not_interested' || category === 'follow_up') ? 'manual_cold' :
      'demoted_from_hot';
    hfuFinish(prevConv, outcome);
    log(`Hot follow-up STOPPED on conv ${convId} — moved to ${category} (${outcome})`);
  }
  // Chris manually dragging warm/cold straight to hot is just as common as the AI
  // auto-submitting one — give it the same address-in-preview treatment. Skip if an
  // address is already on file (e.g. re-dragging an already-graduated lead) so this
  // doesn't fire an extra LLM call for nothing. Deliberately NOT awaited: the category
  // move must resolve to the renderer immediately (the row jumping to HOT is what tells
  // Chris the click landed), the address is a nice-to-have that can pop in a moment later
  // once the next auto-refresh picks it up.
  if (category === 'hot_lead') {
    const conv = db.getConversationById(convId);
    if (conv && !conv.address) {
      (async () => {
        try {
          const settings = db.getAllSettings();
          const msgs = db.getMessages(convId);
          // Full history — see the comment on the autoSubmitLead call for why a trailing
          // slice silently loses the address on threads that go quiet between touches.
          const details = await extractLeadDetails(msgs, settings.claudeApiKey);
          if (details.address) db.setConversationAddress(convId, details.address);
        } catch (err) {
          log(`Manual hot-lead address scan failed for conv ${convId}: ${err.message}`);
        }
      })();
    }
  }
...
// ── Hot Follow-Up Agent: controls ─────────────────────────────────────────────

// The state the toolbar control renders from.
ipcMain.handle('hotfu:get', (_, convId) => {
  const conv = db.getConversationById(convId);
  if (!conv) return null;
  return {
    state: conv.hot_fu_state || null,
    armedAt: conv.hot_fu_armed_at || null,
    nextAt: conv.hot_fu_next_at || null,
    hasNews: !!conv.has_news,
    emoji: conv.emoji || null,
    touches: db.countHotFollowUps(convId),
    // Past the answer but ungraded — running, but nothing will send until it's tagged.
    needsColor: conv.hot_fu_state === 'maintain' && !hfuDealColor(conv),
  };
});

// Arm — the one deliberate activation. Starts Tier 1 press immediately.
ipcMain.handle('hotfu:arm', (_, convId) => {
  const conv = db.getConversationById(convId);
  if (!conv) throw new Error('Conversation not found.');
  // RED HOT is off-limits to every agent — refuse at the door rather than relying on the
...
ipcMain.handle('hotfu:pause', (_, convId) => {
  const conv = db.getConversationById(convId);
  // Remember WHICH phase we interrupted — resume restores it. Only record a real running
  // phase, so pausing twice doesn't overwrite the original with 'paused'.
  const from = (conv && (conv.hot_fu_state === 'press' || conv.hot_fu_state === 'maintain'))
    ? conv.hot_fu_state : (conv && conv.hot_fu_paused_from) || 'maintain';
  db.setHotFuState(convId, 'paused', { hot_fu_next_at: null, hot_fu_paused_from: from });
  db.logAudit('hotfu_paused', { convId, from });
  return true;
});

// Resume. Restores the phase the pause interrupted: a lead paused mid-Tier-1 goes back to
// pressing for an answer, because nothing about the situation changed. Only an actual
// answer promotes a lead to Tier 2 (Phase 2 work — the classifier sets paused_from there).
// The clock re-anchors to NOW, so missed slots are gone rather than owed.
ipcMain.handle('hotfu:resume', (_, convId) => {
  const conv = db.getConversationById(convId);
  if (!conv) throw new Error('Conversation not found.');
  const now = Math.floor(Date.now() / 1000);
  const next = conv.hot_fu_paused_from === 'press' ? 'press' : 'maintain';
  db.setHotFuState(convId, next, { hot_fu_next_at: now, hot_fu_paused_from: null });
  db.logAudit('hotfu_resumed', { convId, phase: next });
  log(`Hot follow-up RESUMED on conv ${convId} as ${next}`);
  return true;
});

ipcMain.handle('hotfu:stop', (_, convId) => {
  const conv = db.getConversationById(convId);
  if (conv) hfuFinish(conv, 'manual_off');
  return true;
});

ipcMain.handle('hotfu:log', (_, convId) => db.getHotFollowUpLog(convId));

// Dry-run the cadence. Pure arithmetic — no DB, no network, cannot send.
ipcMain.handle('hotfu:simulate', (_, opts) => simulateHotFollowUp(opts || {}));

ipcMain.handle('conversations:archive', (_, convId) => {
  // Archiving is another way of saying "I'm done with this thread" — close out follow-up
  // so the state doesn't linger armed behind an archived lead.
  const conv = db.getConversationById(convId);
  if (conv && conv.hot_fu_state && conv.hot_fu_state !== 'stopped') hfuFinish(conv, 'archived');
  db.archiveConversation(convId);
  return true;
});

ipcMain.handle('conversations:delete', (_, convId) => {
  // Record the outcome before the row disappears. hot_lead_outcome has no FK cascade
  // precisely so the analysis history survives a deleted conversation.
  const conv = db.getConversationById(convId);
  if (conv && conv.hot_fu_state && conv.hot_fu_state !== 'stopped') hfuFinish(conv, 'deleted');
...
ipcMain.handle('conversations:sendMessage', async (_, { convId, body }) => {
  ...
  // §8B — the hot agent pauses when YOU speak. Deliberately a separate flag from
  // human_replied above: that one is the warm agent's permanent stand-down, this one is
  // reversible from the toolbar. A lead that isn't armed is unaffected.
  const convRow = db.getConversationById(convId);
  if (convRow && (convRow.hot_fu_state === 'press' || convRow.hot_fu_state === 'maintain')) {
    db.setHotFuState(convId, 'paused', { hot_fu_next_at: null, hot_fu_paused_from: convRow.hot_fu_state });
    log(`Hot follow-up paused on conv ${convId} — you replied in the thread`);
  }
...
ipcMain.handle('settings:save', (_, settings) => {
  // THE ENFORCEMENT POINT. The AI tab's prompt is only presentation; this is what actually
  // stops the AI being enabled, because it cannot be reached from devtools or by editing the
  // database. Attempts are logged so tampering is visible.
  const touched = AI_LOCKED_SETTINGS.filter(k => Object.prototype.hasOwnProperty.call(settings, k));
  if (touched.length) {
    if (!aiLockConfigured()) {
      db.logAudit('ai_lock_missing', { attempted: touched });
      throw new Error('AI features are locked in this build. No unlock password is configured.');
    }
    if (!aiIsUnlocked()) {
      db.logAudit('ai_lock_denied', { attempted: touched });
      throw new Error('Enter the admin password to change AI settings.');
    }
    db.logAudit('ai_lock_change', { changed: touched });
  }
...
ipcMain.handle('ai:unlock', (_, password) => {
  if (!aiLockConfigured()) return { ok: false, reason: 'not_configured' };
  if (!verifyAiLock(password)) {
    db.logAudit('ai_unlock_failed', {});
    return { ok: false, reason: 'bad_password' };
  }
  aiUnlockedUntil = Date.now() + AI_UNLOCK_WINDOW_MS;
  db.logAudit('ai_unlock_ok', {});
  return { ok: true, minutes: AI_UNLOCK_WINDOW_MS / 60000 };
});

ipcMain.handle('ai:lockStatus', () => ({
  configured: aiLockConfigured(),
  unlocked: aiIsUnlocked(),
}));

ipcMain.handle('ai:lock', () => { aiUnlockedUntil = 0; return true; });
