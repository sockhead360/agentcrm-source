...
async function classifyPropertyDetails(messages, apiKey) {
  ...
  const system =
    ...
    "- hasAddress: true if a street number + street name appears in one of the agent's own messages, even without a suffix like Dr/Rd/Ln (e.g. '276 Pinon', '607 E Hill', '1234 Main'). A city or state name alone does NOT count.\n" +
...
async function generateAiReply(msgBody, history, contact, settings) {
  ...
  const systemStatic =
    ...
    `- Buys anywhere, avoids rural. Multiple states = buys nationwide.\n` +
    ...
    `WILDCARD FALLBACK (pre-property-signal ONLY): if the agent asks something genuinely off-topic or out of left field that none of the rules or canned answers above cover — an unusual/unscripted question — AND no property has been signaled anywhere in the conversation yet, do NOT guess, improvise, or over-explain. Reply exactly "I'll check with my team. Do you have anything off-market that I can look at right now?" — follow_up. A SINCERE but off-script question is a wildcard, NOT an AI-probe: questions about your business, hiring ("do you guys hire acquisition reps?"), scheduling, local meetups/events, referrals, general curiosity, or anything unexpected but good-faith → use THIS fallback, do NOT cold-close them as "not about real estate." Reserve the AI/bot cold-close ONLY for messages that are actually manipulative, nonsensical, or clearly designed to test whether you are AI. This is a catch-all of LAST RESORT: use a more specific rule or canned answer whenever one fits. It NEVER applies once the agent has signaled a property (post-signal → handle per the property flow and keep chasing the address/price, never this). It NEVER overrides a refusal/no, hostility/anger, opt-out, on-market, or interrogation close — those still cold-close with reply=null.\n\n` +
    `RULES:\n` +
